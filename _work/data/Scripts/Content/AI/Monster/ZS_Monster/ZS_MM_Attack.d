...
func int ZS_MM_Attack_Loop ()
{
	...
	if (other.aivar[AIV_PARTYMEMBER] == TRUE)
	&& (self.aivar[AIV_PARTYMEMBER] == TRUE)
	{
		return LOOP_END;
	};

	if (self.aivar[AIV_Markiert] > 0)
	{
		if (self.aivar[AIV_Markiert] == 1)
		&& ((C_NpcIsDown(Char_Einhand))
		|| (Npc_GetDistToNpc(self, Char_Einhand) > 1000))
		{
			self.aivar[AIV_Markiert] = 0;
		}
		else if (self.aivar[AIV_Markiert] == 2)
		&& ((C_NpcIsDown(Char_Zweihand))
		|| (Npc_GetDistToNpc(self, Char_Zweihand) > 1000))
		{
			self.aivar[AIV_Markiert] = 0;
		}
		else if (self.aivar[AIV_Markiert] == 3)
		&& (C_NpcIsDown(Char_Bogen))
		{
			self.aivar[AIV_Markiert] = 0;
		}
		else if (self.aivar[AIV_Markiert] == 4)
		&& (C_NpcIsDown(Char_Magie))
		{
			self.aivar[AIV_Markiert] = 0;
		}
		else if (self.aivar[AIV_Markiert] == 5)
		&& (C_NpcIsDown(hero))
		{
			self.aivar[AIV_Markiert] = 0;
		};
	...
	// Spitter vergiftet Gegner und wählt dann nächstes Ziel
	...
	if (Npc_GetDistToNpc(self, Char_Einhand) < FIGHT_DIST_RANGED_OUTER)
	|| (Npc_GetDistToNpc(self, Char_Zweihand) < FIGHT_DIST_RANGED_OUTER)
	|| (Npc_GetDistToNpc(self, Char_Bogen) < FIGHT_DIST_RANGED_OUTER)
	|| (Npc_GetDistToNpc(self, Char_Magie) < FIGHT_DIST_RANGED_OUTER)
	|| (Npc_GetDistToNpc(self, hero) < FIGHT_DIST_RANGED_OUTER)
	{
		self.aivar[AIV_Markiert] = 6;
	};

	if (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Witch))
	{
		if (Npc_GetDistToNpc(self, other) > 300)
		{
			if (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(Char_Magie))
			{
				AI_Teleport	(self, "CHAR_MAGIE");
			};
			if (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(Char_Bogen))
			{
				AI_Teleport	(self, "CHAR_BOGEN");
			};
			if (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(Char_Zweihand))
			{
				AI_Teleport	(self, "CHAR_ZWEIHAND");
			};
			if (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(Char_Einhand))
			{
				AI_Teleport	(self, "CHAR_EINHAND");
			};
			if (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(hero))
			{
